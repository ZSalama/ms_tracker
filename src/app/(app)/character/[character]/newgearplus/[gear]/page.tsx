...
export default async function page({
	...
	let updatedGear: GearItem | undefined
	...
	if (gearData.url) {
		...
		try {
			updatedGear = await prisma.gearItem.update({
				where: { id: gearData.id },
				data: {
					name: data.name ?? gearData.name, // fallback to existing name if not provided
					type: data.type ?? gearData.type, // fallback to existing type if not provided
					rarity: data.rarity ?? 'Unknown', // fallback to 'Unknown' if not provided
					tradeStatus: data.tradeStatus ?? 'untradeable', // fallback to 'untradeable' if not provided
					starForce: data.starForce ?? 0, // fallback to 0 if not provided
					attackPowerIncrease: data.attackPowerIncrease ?? 0, // fallback to 0 if not provided
					combatPowerIncrease: data.combatPowerIncrease ?? 0, // fallback to 0 if not provided
					requiredLevel: data.requiredLevel ?? 0, // fallback to 0 if not provided
